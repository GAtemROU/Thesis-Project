...
df_on_dist <- df %>%
    ...
    mutate(OnDist = OnDist*1) %>%
    select(Subject, Trial, Condition, MsgType, TrgtPos, StrategyLabel, Correct, OnDist)

contrasts(df_on_dist$Condition) <- c(-1, 1)
contrasts(df_on_dist$Condition)

contrasts(df_on_dist$StrategyLabel) <- contr.helmert(3)
contrasts(df_on_dist$StrategyLabel)
...
regression <- glmer(
    OnDist ~ Condition + TrgtPos + Trial +
...
sink(file = "analysis/regressions/MLE/per_fixation/results/on_dist_strategy_labels.txt")
...
ggsave(
    filename = "analysis/regressions/MLE/per_fixation/results/on_dist_strategy_labels_plot.png",
    plot = last_plot(),
    width = 8,
    height = 8,
    dpi = 300
